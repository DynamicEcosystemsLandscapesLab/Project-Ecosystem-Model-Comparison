...
# Model: Sortie-PPA
#=================================================================================================#
model <- SortiePPA$new(file.path(base, "models", "sortie_ppa", "jerc_rd"))
...
model$get_fitness()
jerc_sortie_fitness <- model$fitness
...
unique(model$predictions$c_so$c_so)

# SON constant
model$observations$n_so$n_so
unique(model$predictions$n_so$n_so)
...
# Model: Landis-2
#=================================================================================================#
model <- Landis2$new(file.path(base, "models", "landis_2", "jerc_rd"))
...
# Pre-processing to populate model$observations using get_observations("all")
source("jerc_rd_data.r")
model$set_observations(observation_list, get_observations)
model$files$observations
model$observations
...
# Generate model fitness table
model$get_fitness()
jerc_landis_fitness <- model$fitness
