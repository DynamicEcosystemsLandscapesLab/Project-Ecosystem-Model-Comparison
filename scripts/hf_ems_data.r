...
get_observations <- function(metric="all") {
  funs = list(get_nee(), get_biomass_ag(), get_c_ag(), get_n_ag(), get_biomass_bg(),
              get_c_ag(), get_n_bg(), get_c_so(), get_n_so(), get_r_soil(), get_anpp(),
...
get_r_soil <- function() {
  ...
  df$flux = df$flux / 1e+6 * 24 * 365.24 # convert mg to kg and hour to year
  return(setNames(aggregate(df$flux, by=list(df$year), FUN=mean, na.rm=TRUE), c("year","r_soil")))
}

# Annual net primary productivity (ANPP); Mg year-1; kg year-1
get_anpp <- function() {
  ...
  colnames(df) = tolower(colnames(df))
  df = df[df$site =="ems",]
  df$anpp = df$anpp * 1000 / 10681.42 # Mg year-1 to kg m-2 year-1
